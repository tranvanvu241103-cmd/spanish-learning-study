...
- **Review card now shows a `.qtype-tag` pill** ("🔤 Dịch nghĩa · ES → EN" / "✍️ Dịch nghĩa · EN → ES" / "📝 Điền vào chỗ trống") so the learner always knows which of the three question types they're on, plus a **`💡 Gợi ý` (hint) toggle** (`toggleReviewHint()`) that reveals the vocab item's `explain` field on demand without giving away the answer — scaffolded recall instead of all-or-nothing.
